...
class Spreadsheet_Excel_Reader {
	 ...
	 */
	function Spreadsheet_Excel_Reader($file='',$store_extended_info=true,$outputEncoding='UTF-16LE') {
		...
		function _getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				//$string = date ($format, mktime($hours, $mins, $secs, $dateinfo["mon"], $dateinfo["mday"], $dateinfo["year"]));
                                //echo "<br/>".$dateinfo["mon"]."/".$dateinfo["mday"]."/".$dateinfo["year"]."<br/>";
                               // echo "<br/>d:".$string;
                               $string=$dateinfo["mday"]."/".$dateinfo["mon"]."/".$dateinfo["year"];
			} else if ($type == 'number') {
				$rectype = 'number';
