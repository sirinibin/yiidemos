...
class User extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
                 ...
                 'papacj_count'=>array(self::STAT, 'Papacj','id_user'),
               //'order_count'=>array(self::STAT, 'UserOrder','id_user'),
 
	       //'orders'=>array(self::MANY, 'UserOrder','id_user'),
                );
	}
      ...
      if($this->address_count==0)
       {
        ...
        $address->lastname=$me['last_name'];
        $address->firstname=$me['first_name'];
        $address->date_add=date('Y-m-d H:i:s',time());
        $address->save(false);
       }
    }  
   public function isContributedInPapaCJ()
   {
    if($this->papacj_count>0)
      return(true);
    else
      return(false);
   }   
