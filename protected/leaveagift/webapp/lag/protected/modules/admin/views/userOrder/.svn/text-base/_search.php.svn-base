<?php  $form=$this->beginWidget('bootstrap.widgets.TbActiveForm',array(
        'id'=>'search-user-order-form',
        'enableAjaxValidation'=>false,
	'action'=>Yii::app()->createUrl($this->route),
	'method'=>'get',
));  ?>

       <?php //echo CHtml::label('Date between', CHtml::activeId($model, 'date_add'), array('class'=>'span2')); ?>
       <?php  echo $form->textField($model,'date_add', array('class'=>'span2'));
   ...
   'value'=>(isset(Yii::app()->request->cookies['to_date'])) ? Yii::app()->request->cookies['to_date']->value : '',
    ...
    'htmlOptions'=>array(
       ...
       <?php
              ...
              echo $form->textField($model,'endDate', array('class'=>'span2'));
          /*     $this->widget('zii.widgets.jui.CJuiDatePicker', array(
              'model'=>$model,
    'name'=>'endDate',
   'value'=>(isset(Yii::app()->request->cookies['to_date'])) ? Yii::app()->request->cookies['to_date']->value : '',
	...
	<?php //echo $form->textFieldRow($model,'id_user_gift',array('class'=>'span5','maxlength'=>64)); ?>

	<?php //echo $form->textFieldRow($model,'id_cart',array('class'=>'span5','maxlength'=>64)); ?>

        <?php echo $form->dropDownListRow($model,'id_product',$model->getTagOptions(),array('empty'=>array(''=>'Select a Product'))); ?>
...
?>	
   <script>
	...
	$(".btnreset").click(function(){
		$(":input","#search-user-order-form").each(function() {
		var type = this.type;
...
var dates = $( "#'.CHtml::activeId($model, 'date_add').', #'.CHtml::activeId($model, 'endDate').'" ).datepicker({
		changeMonth: true,
		onSelect: function( selectedDate ) {
			var option = this.id == "'.CHtml::activeId($model, 'date_add').'" ? "minDate" : "maxDate",
