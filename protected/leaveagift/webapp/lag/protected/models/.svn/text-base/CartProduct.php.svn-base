...
/**
 * This is the model class for table "ps_cart_product".
 *
 * The followings are the available columns in table 'ps_cart_product':
...
class CartProduct extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id_cart, id_product, id_product_attribute', 'length', 'max'=>64),
			array('quantity', 'length', 'max'=>10),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_cart, id_product, id_product_attribute, quantity, date_add', 'safe', 'on'=>'search'),
		);
	}
